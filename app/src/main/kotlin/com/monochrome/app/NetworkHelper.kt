...
object NetworkHelper {
    ...
    fun injectHooks(request: WebResourceRequest): WebResourceResponse? {
        return try {
            ...
            val charset = try { Charset.forName(ct.substringAfter("charset=", "").trim().ifBlank { "UTF-8" }) } catch (_: Exception) { Charsets.UTF_8 }
            val originalHtml = conn.inputStream.bufferedReader(charset).use { it.readText() }
            
            val script = "<script>$JS_HOOKS</script>"
    ...
    fun proxyWithCors(request: WebResourceRequest, method: String): WebResourceResponse? {
        return try {
            ...
            request.requestHeaders?.forEach { (k, v) ->
                if (!k.equals("User-Agent", true) && !k.equals("Host", true) && !k.equals("Origin", true) && !k.equals("Referer", true) && !k.equals("X-Requested-With", true) && !k.equals("Cookie", true))
            ...
            if (mime == "text/html") {
                ...
                if (path.endsWith(".js")) mime = "application/javascript" else if (path.endsWith(".css")) mime = "text/css"
            }
            ...
            conn.headerFields.forEach { (k, v) ->
                ...
                if (k != null && !restrictedHeaders.any { it.equals(k, ignoreCase = true) }) {
                    ...
                    if (k.equals("Location", true)) value = value.replace("redirect_uri=http%3A%2F%2Fauth.", "redirect_uri=https%3A%2F%2Fauth.")
                    respHeaders[k] = value
                }
            }
